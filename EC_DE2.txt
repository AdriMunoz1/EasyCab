...
sensor_socket = None  # Socket para el sensor
...
def handle_central_commands(central_socket, taxi_id):
    global taxi_status, taxi_position, destination_position, taxi_thread, sensor_socket  # Para actualizar el estado del taxi
    try:
        while True:
            ...
            if message.startswith("STOP#"):
                ...
                if sensor_socket:
                    sensor_socket.send(b'RESUME')  # Enviar mensaje de reanudación a EC_S
    ...
    try:
        ...
        while True:
            ...
            if signal == "KO":
                ...
                with taxi_status_lock:
                    taxi_status = "detenido"  # Detener el taxi
                client_socket.send("KO".encode("utf-8"))
                time.sleep(5)  # Pausa de 5 segundos
                msg = f"RESUME#{taxi_id}"
                central_socket.send(msg.encode("utf-8"))
                with taxi_status_lock:
                    taxi_status = "en movimiento"  # Reanudar el taxi
            #else:
